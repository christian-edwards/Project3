    ...
    while(exit){
      ...
      inLine.split(" ")(0).toUpperCase() match{
        ...
        case "QUERY6" =>
          ...
          df.sparkSession.sql(s"""select product_category, sum(qty*cast(replace(price, "$$", "") as decimal(10, 2))) as Sales from global_temp.InputData where payment_txn_success == 'Y' group by product_category order by Sales asc""").show(false)
        case "EXIT" =>
          return
        case default =>
          try {
